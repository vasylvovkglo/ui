...
function getColumnValues(driver, table, columnName) {
    return driver.findElements(table.tableColumns[columnName])
...
const action = {
    ...
    isContainsValueInColumn: async function (driver, table, columnName, value) {
        await getColumnValues(driver, table, columnName)
            .then(arr => { return arr.includes(value) })
    ...
    findRowIndexesByColumnValue: async function (driver, table, columnName, value) {
        return await getColumnValues(driver, table, columnName)
            .then(arr => {
                let indexes = []
                for (let indx in arr) {
                    if (arr[indx] === value) {
    ...
    openActionMenuByIndex: async function (driver, table, index) {
        return await driver.findElements(table.tableFields['action_menu'](index))
            .then(elements => {
                if (elements.length === 0) {
                    return driver.findElement(table.tableFields['action_menu_button'](index));
                }
            })
            .then (element => {
                if (element) {
